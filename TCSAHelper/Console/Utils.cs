...
public static class Utils
{
    ...
    public static PaginationResult DeterminePagination(int heightAvailableToList, int listLength, int heightPerItem = 1, int perPageListHeightOverhead = 0, int skippedItems = 0)
    {
        ...
        if (listLength < 0) throw new ArgumentException("List length must be a positive number.");
        if (heightPerItem < 1) throw new ArgumentException("Height per item must be a positive number.");
        if (perPageListHeightOverhead < 0) throw new ArgumentException("Per page list height overhead must be a positive number.");
        if (skippedItems < 0) throw new ArgumentException("Skipped items must be a positive number.");
